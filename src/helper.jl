...
function gen_randpots(rs::Array, seed::Int=123)::Vector{Tuple{Vector, Float64}}
	...
	for conf in Iterators.product(rs...)
		push!(potentials, ([conf...], rand(1.0:1.0:10.0)))
	end
...
"""
	scale_factor!(f::Factor, alpha::Float64)

Scale the potentials of the factor `f` by the scalar `alpha`.
"""
function scale_factor!(f::Factor, alpha::Float64)
	@assert alpha >= 0
	for (key, _) in f.potentials
		f.potentials[key] *= alpha
	end
end

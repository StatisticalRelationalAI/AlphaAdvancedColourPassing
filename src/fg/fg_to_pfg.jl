...
function groups_to_pfg(
	...
	for (f_group_id, f_group) in factor_groups
		...
		if num_rvs == num_prvs # Number of neighbors unchanged: No CRV
			...
			# Cummutative RVs are only a subset and buckets in cache
			if length(commutative_rvs) != length(rvs(f_group[1])) ||
					!haskey(hist_cache, f_group[1])
				counted_ps = buildhistograms(f_group[1], commutative_rvs)
			else
				counted_ps, _ = hist_cache[f_group[1]]
			end
...
"""
function buildhistograms(f::DiscreteFactor, args::Vector{DiscreteRV})::Dict
